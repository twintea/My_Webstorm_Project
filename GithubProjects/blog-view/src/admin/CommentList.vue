<template>
  <div>
    ...
    <el-row>
      <el-col :span="6">
        <el-select v-model="queryInfo.articleId" placeholder="请选择页面" :filterable="true" :clearable="true"
                   @change="search" size="small" style="min-width: 400px">
    ...
    <el-table :data="commentList" row-key="id" :tree-props="{children: 'replyComments'}">
      ...
      <el-table-column label="博客标题" show-overflow-tooltip>
        <template v-slot="scope">
          <el-link type="success" :href="`http://localhost:8080/#/DetailArticle?aid=${scope.row.articleId}`">
            {{title}}
          </el-link>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="200">
        <template v-slot="scope">
          ...
          <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteCommentById(scope.row.id)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                   ...
                   :page-sizes="[10, 20, 30, 50]" :page-size.sync="queryInfo.pageSize" :total="total"
    ...
    <el-dialog title="编辑评论" width="50%" :visible.sync="editDialogVisible" :close-on-click-modal="false"
      ...
      <el-form :model="editForm" :rules="formRules" ref="editFormRef" label-width="80px">
        <el-form-item label="昵称" prop="nickname">
          <el-input disabled="true" v-model="editForm.nickname"></el-input>
        </el-form-item>
        <el-form-item label="头像" prop="avatar">
          <el-input disabled="true" v-model="editForm.avatar"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getCommentList() {
      ...
      this.$axios.get('/commentList?currentPage=' + this.queryInfo.pageNum + "&pageSize=" + this.queryInfo.pageSize, {
        ...
        for (var i in _this.blogList) {
          for (var j in _this.commentList) {
            if (_this.blogList[i].id == _this.commentList[j].articleId){
             this.title = _this.blogList[i].title
            }
          }
        }
      });
    ...
    search() {
      const _this = this
      this.$axios.get('/comment/detail?articleId=' + this.queryInfo.articleId + "&currentPage=" + this.queryInfo.pageNum + "&pageSize=" + this.queryInfo.pageSize, {
        headers: {
          "Authorization": localStorage.getItem("token")
        }
      }).then((res) => {
    ...
    open3() {
      this.$message.warning('该文章下没有评论噢');
    },
